...
class CommunityAdminModelCourse extends JModelLegacy
{
	/**
	 * Configuration data
	 *
	...
	public function _buildQuery()
	{
		...
		$condition	= '';
		$ordering		= $mainframe->getUserStateFromRequest( "com_community.course.filter_order",		'filter_order',		'a.Name',	'cmd' );
		$orderDirection	= $mainframe->getUserStateFromRequest( "com_community.course.filter_order_Dir",	'filter_order_Dir',	'',			'word' );
	...
	public function getSelectCourse($courseId)
	{
		$mainframe	= JFactory::getApplication();

		$db		= JFactory::getDBO();

		$query	= 'SELECT * FROM ' . $db->quoteName( '#__gwm_courses')
				. 'Where `uid`='. $db->Quote( $courseId );
	...
	public function getAllUsers($orderBy='')
	{
		...
		$query	= "SELECT * FROM " . $db->quoteName( '#__users').$sortQuery;
	...
	public function getUpdateCourse($fieldset)
	{
		$mainframe	= JFactory::getApplication();
        $jinput = $mainframe->input;
		//print_r($fieldset);
		...
		$query	= 'UPDATE '. $db->quoteName('#__gwm_courses')
            .' SET ' . $db->quoteName('Name').' = '.$db->Quote($fieldset['Name'])
			.' , ' . $db->quoteName('Address1').' = '.$db->Quote($fieldset['Address1'])
			.' , ' . $db->quoteName('Address2').' = '.$db->Quote($fieldset['Address2'])
			.' , ' . $db->quoteName('City').' = '.$db->Quote($fieldset['City'])
			.' , ' . $db->quoteName('State').' = '.$db->Quote($fieldset['State'])
			.' , ' . $db->quoteName('ZipCode').' = '.$db->Quote($fieldset['ZipCode'])
			.' , ' . $db->quoteName('Phone').' = '.$db->Quote($fieldset['Phone'])
			.' , ' . $db->quoteName('BPhone').' = '.$db->Quote($fieldset['BPhone'])
			.' , ' . $db->quoteName('URL').' = '.$db->Quote($fieldset['URL'])
			.' , ' . $db->quoteName('BURL').' = '.$db->Quote($fieldset['BURL'])
			.' , ' . $db->quoteName('locMap').' = '.$db->Quote($fieldset['locMap'])
			.' , ' . $db->quoteName('priceRange').' = '.$db->Quote($fieldset['priceRange'])
			.' , ' . $db->quoteName('Holes').' = '.$db->Quote($fieldset['Holes'])
			.' , ' . $db->quoteName('CourseType').' = '.$db->Quote($fieldset['CourseType'])
			.' , ' . $db->quoteName('Status').' = '.$db->Quote($fieldset['Status'])
			.' , ' . $db->quoteName('ownerid').' = '.$db->Quote($fieldset['ownerid'])
			.' , ' . $db->quoteName('Updated').' = '.$db->Quote($date->toSql())
			.' WHERE '.$db->quoteName('uid').'='.$db->Quote($fieldset['id']);
