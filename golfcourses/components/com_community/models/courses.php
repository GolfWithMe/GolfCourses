...
class CommunityModelCourses extends JCCModel
    implements CLimitsInterface, CNotificationsInterface
{
	...
	public function getMyCourse( $userId, $fields)
	{
		$db		= $this->getDBO();
		...
		$extraSQL	= ' AND a.ownerid=' . $db->Quote($userId);
		
		if($fields['searchkey'] != undefined && $fields['searchkey']!='') {
			$extraSQL .= ' AND a.Name Like "%' . $fields['searchkey'] .'%"';
		}
	...
	public function getUpdateCourse($fieldset)
	{
		...
		$query	= 'UPDATE '. $db->quoteName('#__gwm_courses')
			...
			.' , ' . $db->quoteName('CourseType').' = '.$db->Quote($fieldset['CourseType'])
                        .' , ' . $db->quoteName('HomeGroup').' = '.$db->Quote($fieldset['HomeGroup'])
	...
	public function updateRequestorAccept($id, $fieldset)
    {
		...
		$query	= 'UPDATE '. $db->quoteName('#__gwm_group_schedule')
            .' SET ' . $db->quoteName('teetime').' = '.$db->Quote($fieldset['teetime'])
